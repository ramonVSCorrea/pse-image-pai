...
class ImageProcessor:
    ...
    def clamp_pixel(self, value: int) -> int:
        if value < 0:
            return 0
        if value > 255:
            return 255
    ...
    def process_convolution(self, node: Dict, inputs: List) -> Dict:
        ...
        filter_type = str(params.get('filterType', 'convolution')).lower()  # Tipo de filtro
        
        # Redireciona para filtros especializados
        if filter_type in ('mediana', 'median'):
            return self.process_median(pixels, width, height, kernel_size)
        
        if filter_type in ('laplacian', 'laplaciano'):
            return self.process_laplacian(pixels, width, height, kernel_size)
        
        if filter_type in ('media', 'average', 'mean'):
        ...
        for y in range(height):
            for x in range(width):
                ...
                output[y * width + x] = self.clamp_pixel(result)
    ...
    def process_laplacian(self, pixels: List[int], width: int, height: int, window_size: int = 3) -> Dict:
        """
        ═══════════════════════════════════════════════════════════════
        ...
        ═══════════════════════════════════════════════════════════════
        """
        window_size = window_size if window_size % 2 == 1 and window_size >= 3 else 3
        ...
        if window_size == 3:
            ...
            kernel = [[0 for _ in range(window_size)] for _ in range(window_size)]
            for i in range(window_size):
                if i != center:
                    kernel[center][i] = -1
                    kernel[i][center] = -1
            kernel[center][center] = (window_size - 1) * 2
        ...
        for y in range(height):
            for x in range(width):
                ...
                result = self.clamp_pixel(accumulator)
    ...
    def process_point_operation(self, node: Dict, inputs: List) -> Dict:
        ...
        if operation == 'brightness':
            ...
            for i in range(len(pixels)):
                output[i] = self.clamp_pixel(pixels[i] + brightness)
    ...
    def process_difference(self, node: Dict, inputs: List) -> Dict:
        ...
        for i in range(len(pixels1)):
            difference = pixels1[i] - pixels2[i]
            if difference < 0:
    ...
    def process_save(self, node: Dict, inputs: List) -> Dict:
        """
        Salva imagem em formato PGM P5 binário
        """
        if not inputs:
            return {"error": "Nenhuma entrada para salvar"}
        
        image_data = inputs[0]
        filename = node.get('data', {}).get('filename', 'output.pgm')
        ...
        if width <= 0 or height <= 0 or len(pixels) != width * height:
            return {"error": "Imagem inválida para salvar em PGM"}
        
        try:
            header = f"P5\n{width} {height}\n255\n".encode('ascii')
            output_bytes = bytearray()
            for pixel in pixels:
                output_bytes.append(self.clamp_pixel(int(pixel)))
            pixel_bytes = bytes(output_bytes)
            
            import os
            save_path = os.path.join(os.path.dirname(__file__), 'output', filename)
            ...
            return {
                ...
                "saved": True,
                "message": f"Arquivo PGM salvo em {save_path}"
            }
        except Exception as e:
            return {
                "type": "save",
